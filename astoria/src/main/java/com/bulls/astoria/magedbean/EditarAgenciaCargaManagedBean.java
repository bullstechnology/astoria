...
@ManagedBean (name="editarCargaMB")
@SessionScoped
public class EditarAgenciaCargaManagedBean extends GeneralManagedBean implements Serializable{
	private Integer id;
	...
	List <com.bulls.astoria.pojo.Destino> listaDestinos;
	...
	@PostConstruct
	public void EditarAgenciaCargaManagedBean(){
		borrarSession();
		...
		agencias = dominioService.getAgenciasCarga();
    ...
    public String editar (AgenciaCarga agencia){
    	
  	  	 AgenciaCarga agenciaedit = new AgenciaCarga();
  	  	 agenciaedit = dominioService.getAgenciaXId(agencia.getId());
 		 this.setAgencia(agenciaedit);
...
public void getDestinos (){
    ...
    public void setAgencia(AgenciaCarga agencia){
      this.id = agencia.getId();	  
  	  this.cel1 = agencia.getCel1();
   	  this.cel2 = agencia.getCel2();
   	  this.ciudad = agencia.getCiudad();
   	  this.codigo = agencia.getCodigo();
   	  this.contacto1 = agencia.getContacto1();
   	  this.contacto2 = agencia.getContacto2();
   	  this.correo1 = agencia.getCorreo1();
   	  this.correo2 = agencia.getCorreo2();
   	  this.cupo = agencia.getCupo();
   	  this.desccupo = agencia.getDesccupo();
   	  this.descsaldofinal = agencia.getDescsaldofinal();
   	  this.direccion = agencia.getDireccion();
   	  this.fax = agencia.getFax();
   	  this.nombre = agencia.getNombre();
   	  this.numident = agencia.getNumident();
   	  this.observaciones = agencia.getObservaciones();
   	  this.infobancaria = agencia.getInfobancaria();
   	  this.pais = agencia.getPais();
   	  this.saldofinal = agencia.getSaldofinal();
   	  this.saldoinicial = agencia.getSaldoinicial();
	  this.skype1 = agencia.getSkype1();
	  this.skype2 = agencia.getSkype2();
	  this.tel1 = agencia.getTel1();
	  this.tel2 = agencia.getTel2();
	  this.tipoident = agencia.getTipoident();
	  this.web = agencia.getWeb();
	  this.enabled = agencia.isEnabled();
	  	  
	  getCiudadesPais();
      //this.listaDestinos = setDestinos(agencia);
	  listaDestinostmp=setDestinos(agencia);
      return;
    }
...
private List<com.bulls.astoria.pojo.Destino> setDestinos(AgenciaCarga agencia){
	
	Set<Destino> targeDestinos = agencia.getDestino();
	...
	for (Destino destino : list){
		...
		//inicio destino nuevo
		
		com.bulls.astoria.pojo.Destino destinoaux = new com.bulls.astoria.pojo.Destino();
		Ruta ruta = dominioService.getRutaXId(Integer.parseInt(Integer.toString(destino.getIdruta())));

		destinopojo.setId(destino.getIdruta());
		destinopojo.setIddestino(destino.getIddestino());
		destinopojo.setIdorigen(destino.getIdorigen());
		destinopojo.setIdciudaddestino(destino.getIdciudaddestino());
		destinopojo.setIdciudadorigen(destino.getIdciudadorigen());
		...
		destinopojo.setRangofinal2(destino.getRangofinal2());
		destinopojo.setRangoinicial2(destino.getRangoinicial2());
		destinopojo.setTarifaastoria2(destino.getTarifaastoria2());
		destinopojo.setTarifacliente2(destino.getTarifacliente2());
		
		
		destinopojo.setRangofinal3(destino.getRangoinicial3());
		destinopojo.setRangoinicial3(destino.getRangoinicial3());
		destinopojo.setTarifaastoria3(destino.getTarifaastoria3());
		destinopojo.setTarifacliente3(destino.getTarifacliente3());
		
		
		
		
	
		destinopojo.setNombreorigen(dominioService.getDominio(ruta.getIdpaisorigen()).getNomcorto());
		destinopojo.setNombrefinal(dominioService.getDominio(ruta.getIdpaisdestino()).getNomcorto());
		
		destinopojo.setNombreciudad(dominioService.getDominio(ruta.getIdciudadorigen()).getNomcorto());
		destinopojo.setNombreciudadfinal(dominioService.getDominio(ruta.getIdciudaddestino()).getNomcorto());
		
		destinopojo.setAerolinea(Integer.parseInt(Integer.toString(destino.getIdaerolinea())));
		destinopojo.setNombreaerolinea(dominioService.getAerolineaXId(destino.getIdaerolinea()).getNombre());
		destinopojo.setLunes(destino.isLunes());
		destinopojo.setMartes(destino.isMartes());
		destinopojo.setMiercoles(destino.isMiercoles());
		destinopojo.setJueves(destino.isJueves());
		destinopojo.setViernes(destino.isViernes());
		destinopojo.setSabado(destino.isSabado());
		destinopojo.setDomingo(destino.isDomingo());
		destinopojo.setConcepto1(destino.getConcepto1());
		destinopojo.setValor1(destino.getValor1());
		destinopojo.setValorast1(destino.getValorast1());
		destinopojo.setConcepto2(destino.getConcepto2());
		destinopojo.setValor2(destino.getValor2());
		destinopojo.setValorast2(destino.getValorast2());
		destinopojo.setConcepto3(destino.getConcepto3());
		destinopojo.setValor3(destino.getValor3());
		destinopojo.setValorast3(destino.getValorast3());
		destinopojo.setConcepto4(destino.getConcepto4());
		destinopojo.setValor4(destino.getValor4());
		destinopojo.setValorast4(destino.getValorast4());
		destinopojo.setObservaciones(destino.getObservaciones());
	
		listaDestinostmp.add(destinopojo);
    ...
    public String editarAgencia(){
	  ...
	  agencia.setSaldofinal(saldofinal);
	  agencia.setSaldoinicial(saldoinicial);
	  ...
	  agencia.setWeb(web);
	  agencia.setEnabled(this.enabled);
	
    		
    		
    		if(validarAgencia()){
    			agencia.setDestino(putDestinos(agencia));
    			dominioService.editarAgencia(agencia);
    			this.putAuditoria("Editar Agencia de carga", "U", "Edito la agencia de carga así : - " + agencia.toString());
    			agencias = dominioService.getAgenciasCarga();
    			listaDestinostmp = listaDestinostmp = new ArrayList <com.bulls.astoria.pojo.Destino> ();
    			abrirConfirmacion();
...
/*public void subir (){
    	...
    	if(validarSubir()){
    		...
    		destino.setIdciudaddestino(this.idciudaddestino);
    		destino.setIdciudadorigen(this.idciudadorigen);
    		...
    		destino.setNombreciudad(dominioService.getDominio(this.idciudadorigen).getNomcorto());
    		destino.setNombreciudadfinal(dominioService.getDominio(this.idciudaddestino).getNomcorto());
...
public void subir (){
	...
	if(validarSubir()){
		...
		com.bulls.astoria.pojo.Destino destino = new com.bulls.astoria.pojo.Destino();
		//Ruta ruta = dominioService.getRutaXId(Integer.parseInt(this.idzdestino));
		Ruta ruta = dominioService.getRutaXId(this.iddestino);
...
public void resetDestino(){
	this.tarifaguia=null;
	this.rangofinal=null;
	this.rangoinicial=null;
	this.tarifaastoria=null;
	this.tarifacliente=null;
	this.rangofinal2=null;
	this.rangoinicial2=null;
	this.tarifaastoria2=null;
	this.tarifacliente2=null;
	this.rangofinal3=null;
	this.rangoinicial3=null;
	this.tarifaastoria3=null;
	this.tarifacliente3=null;
    this.lunes=false;
	this.martes=false;
	...
	this.valor4=null;
	this.valorast4=null;
    this.observacionesdestino=null;
    this.aerolinea=null;
    this.iddestino=null;
}
   ...
   public boolean validarSubir(){
	  	  ...
	  	  if(this.rangofinal == null)
	  		this.rangofinal =0;
	  	  if(this.rangoinicial2 == null)
	  		this.rangoinicial2 =0;  
	  	  if(this.rangofinal2 == null)
	  		this.rangofinal2  = 0;
	  	  if(this.rangoinicial3 == null)
	  		this.rangoinicial3 =0; 
	  	  if(this.rangofinal3 == null)
	  		this.rangofinal3=0; 
		  ...
		  if(!ok){
	  	  ...
	  	  //fin valida rangos
	    	  ...
	    	  for (com.bulls.astoria.pojo.Destino pojo : listaDestinostmp){
	    		  ...
	    		  if((pojo.getAerolinea().intValue() == Integer.parseInt(this.aerolinea)) && (pojo.getId() == this.iddestino)){
	  		    	ok=false;
	  		    	FacesContext.getCurrentInstance().addMessage(null,
    ...
    private void abrirConfirmacion(){
		...
		FacesMessage message = new FacesMessage(FacesMessage.SEVERITY_INFO, bundle.getString("tituloconfirmacion"), bundle.getString("confirmaeditaragencia"));
    ...
    public Double getSaldoAgencia(AgenciaCarga agencia){
		Double saldo = 0.0;
		saldo = dominioService.getSaldo(agencia.getId(), "A","A");
		if(saldo==null){
			saldo = 0.0;
		}
		return saldo;
