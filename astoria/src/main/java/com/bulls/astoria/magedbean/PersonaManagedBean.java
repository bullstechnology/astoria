...
import com.bulls.astoria.persistence.UserRole;
...
public class PersonaManagedBean extends GeneralManagedBean implements Serializable{
	...
	@PostConstruct
	public void personaManagedBean(){
	 ...
	 public String addPersona() {
	        try {
	        	...
	        	if(this.validar()){
	        		...
	        		persona.setSegnombre(getSegnombre());
	        		persona.setApellido(getApellido());
	        		persona.setSegapellido(getSegapellido());
	        		persona.setTipodoc(getTipodoc());
	        		persona.setNumdoc(getNumdoc());
	        		...
	        		persona.setEmpleado(this.getEmpleado());
	        		...
	        		persona.setIdHandler(this.handler);
	        		...
	        		getPersonaService().savePersona(persona, user);
	        		this.putAuditoria("Crear Usuario", "C", "creo el usuario : - " + persona.toString()  +   "  " + username.toString());
	        		reset();
	        		abrirConfirmacion();
	        	}	
	            return SUCCESS;
		...
		boolean ok = true;
			...
			if (!Validador.esLongitudCorrecta(this.nombre, 1, 45)){
		   	...
		   	if (userService.getUser(this.username) != null){
		   		FacesContext.getCurrentInstance().addMessage(null,
						new FacesMessage(FacesMessage.SEVERITY_ERROR,"Usuario : "," El usuario ya existe" ));
				ok = false;
		   	}
	...
	public boolean getValidarChecks(){
		...
		if(this.getEscliente() && this.getCliente().intValue()== 0){
			ok=false;
			
		}else if(this.getEsplantacion() && this.getPlantacion().intValue()== 0){
			ok=false;
			
		}else if(this.getEscarguera() && this.getCarguera().intValue()== 0){
			ok=false;
			
		}else if(this.getEmpleado() && this.getCargo().intValue()== 0){
			ok=false;
		}else if(this.getEshandler() && this.getHandler().intValue()== 0){
			ok=false;
		}
